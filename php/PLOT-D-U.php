  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav ms-auto">
       <li class="nav-item">
         <a class="nav-link" href="PLOT-A-U.php">Block A</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="PLOT-B-U.php">Block B</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="PLOT-C-U.php">Block C</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="PLOT-D-U.php">Block D</a>
        </li>
      </ul>
    </div>
  </div>
...
<br>
  ...
  <h6> Search the by Name Deceased Person</h6>
...
WHERE a.`Block_id` = 4";
        ...
        $result = $conn->query($sqlCemeteryRecord);
...
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
  $id = $row["Grave_ID"];
  $status = $row["Status"];
  $nameDeceased = $row["Name_Deceased"];  // Fetch the Name_Deceased

  echo '<div class="grid-item">';
  echo '<a href="TestGraveDetails-UserView.php?cmid=' . $id . '" class="btn btn-primary" style="text-decoration: none;">' . $id . '</a>';
  echo '<p>Name: ' . $nameDeceased . '</p>';  // Display the Name_Deceased
  // Change color based on status and make it bold
  echo '<p><strong style="color: ' . ($status == 'Available' ? 'green' : 'red') . ';">Status: ' . $status . '</strong></p>';
  echo '</div>';
}
      ...
      // Display a grid item for "Not Available"
...
<script>
  ...
  function filterItems() {
    ...
    Array.from(items).forEach((item) => {
      const linkText = item.getElementsByTagName('a')[0].textContent.toLowerCase();  // Grave_ID
      const nameText = item.getElementsByTagName('p')[0].textContent.toLowerCase();  // Name_Deceased
      
      if (linkText.includes(searchTerm) || nameText.includes(searchTerm)) {
